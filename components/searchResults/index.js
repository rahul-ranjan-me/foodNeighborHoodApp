...
export default function SearchResults(props) {
  ...
  const showResult = (result, key) => {
    ...
    return (
      <TouchableOpacity style={styles.resultContainer} key={key} onPress={() => props.navigation.navigate('Details', {chefId: String(chefId)})}>
  ...
  const showEmpty = () => {
    return (
      <View style={styles.emptyResult}>
        <Entypo name="emoji-happy" size={130} color={colors.primaryCallAction} />
        <Text style={styles.refineSearch}>Please refine your search term so that we can serve best food to you.</Text>
      </View>
    )
  }
