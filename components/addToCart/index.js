...
export default function AddToCart(props) {
  ...
  useEffect(() => {
    ...
    if (quantity > 0) {
      if (
        newCart.items &&
        Object.keys(newCart.items).length &&
        newCart.chefId === props.details.chefId
      ) {
      ...
      <View style={styles.container}>
        ...
        <View style={styles.cartQuantity}>
          <TextInput
            ...
            style={{ width: 30, fontSize: 22, textAlign: "center" }}
          />
        </View>
  ...
  return <View>{quantity < 1 ? showBuyNow() : changeQuantity()}</View>;
}
