...
export default function Checkout(props) {
  const { usersDetails, setBillAmount, billAmount, setCart, cart } =
  ...
  const messageToRestaurant = () => {
    return (
      <View style={styles.messageContainer}>
        <TextInput
          placeholder="Message for the chef"
  ...
  const calcTotalPrice = (item) => {
    ...
    return (
      ...
      >{`${itemQuantity} x ${price} = ₹ ${itemPrice}`}</Text>
    );
  };
      ...
      <View style={styles.addressContainer}>
        <View style={{ flex: 2 }}>
          ...
          <Text style={{ fontWeight: "bold" }}>
            {address ? address : "Address not entered"}
          </Text>
        </View>
